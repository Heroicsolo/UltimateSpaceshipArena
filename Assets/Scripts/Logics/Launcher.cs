...
public class Launcher : MonoBehaviourPunCallbacks, IMatchmakingCallbacks
{
    ...
    public int OnFightWon(int place = 1, int score = 0)
    {
        ...
        AccountManager.Currency += moneyGained;
        AccountManager.Exp += BalanceProvider.Balance.expPerFightMin + Mathf.CeilToInt((1 + score) * BalanceProvider.Balance.expPerScoreModifier);
    ...
    public void OnDailyRewardsChanged(int lastDailyRewardDebugTime, string lastDailyRewardTime, int lastDailyReward)
    {
        ...
        NotificationsManager.SendNotification("Notification_DailyReward", 1440, 10001);
    }
